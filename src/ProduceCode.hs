...
-}
 ...
 import Data.IOArray ( IOArray, newIOArray, readIOArray, writeIOArray, sizeIOArray )
 import Data.Array( Array, bounds, assocs, elems, (!), array, listArray )
...
-}
 ...
 mkTables action goto first_nonterm fst_term 
  ...
  where 
     ...
     (table,check,act_offs,goto_offs,max_off) 
         = unsafePerformIO (genTables (length actions) max_token sorted_actions)
 ...
 freeze :: IOArray a -> IO (Array Int a)
 freeze io = do
   aList <- elemsIOArray io
   pure $ array (0, length aList - 1) (zip [0..] aList)
 ...
 genTables n_actions max_token entries = do
   ...
   table      <- newIOArray (mAX_TABLE_SIZE) 0
   check      <- newIOArray (mAX_TABLE_SIZE) (-1)
   act_offs   <- newIOArray (n_actions) 0
   goto_offs  <- newIOArray (n_actions) 0
   off_arr    <- newIOArray (mAX_TABLE_SIZE+max_token) 0
 ...
 genTables'
     :: IOArray Int        -- table
     -> IOArray Int        -- check
     -> IOArray Int        -- action offsets
     -> IOArray Int        -- goto offsets
     -> IOArray Int        -- offset array
     -> [TableEntry]        -- entries for the table
     -> Int                -- maximum token no.
     -> IO Int                -- highest offset in table

 genTables' table check act_offs goto_offs off_arr entries max_token
    = fit_all entries 0 1
   where

     fit_all :: [TableEntry] -> Int -> Int -> IO Int
     ...
     fit_all (s:ss) max_off fst_zero = do
       ...
       writeIOArray off_arr (max_token+off) 1
     ...
     same_states s@(_,_,_,_,_,acts) ss@((e,no,_,_,_,acts'):ss') off
       | acts == acts' = do writeIOArray (which_off e) no off
     ...
     fit (act_or_goto, state_no, deflt, _, _, state@((t,_):_)) 
        max_off fst_zero = do
         ...
         -- offset zero.
       off <- findFreeOffset (-t+fst_zero) check off_arr state max_token
       ...
       writeIOArray (which_off act_or_goto) state_no off
...
slot is free or not. -}
 ...
 findFreeOffset :: Int -> IOArray Int -> IOArray Int -> [(Int,Int)] -> Int -> IO Int
 findFreeOffset off table off_arr state max_token = do
     -- offset 0 isn't allowed
     if off == 0 then try_next else do

       -- don't use an offset we've used before
       b <- readIOArray off_arr (max_token+off)
  ...
  where
     try_next = findFreeOffset (off+1) table off_arr state max_token


 fits :: Int -> [(Int,Int)] -> IOArray Int -> IO Bool
 fits off [] table = return True
 fits off ((t,_):rest) table = do
   i <- readIOArray table (off+t)
 ...
 addState off table check ((t,val):state) = do
    writeIOArray table (off+t) val
    writeIOArray check (off+t) t
    addState off table check state
 ...
 findFstFreeSlot :: IOArray Int -> Int -> IO Int
 findFstFreeSlot table n = do
     i <- readIOArray table n
